...
--
-- Groups files with EXIF by Make, Model and BodySerialNumber for
-- scans equal to N, which is defaulted to the last scan ID, if
-- omitted.
--
SELECT
    Make,
...
WHERE
    ...
    BodySerialNumber IS NOT NULL
GROUP BY
    exif.Make COLLATE NOCASE, exif.Model COLLATE NOCASE, BodySerialNumber
ORDER BY
    COUNT(exif_id) DESC
